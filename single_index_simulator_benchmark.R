## Benchmark: Single-Index Model Simulator Optimization
## Tests before/after performance of parametric outcome simulation
## Isolates only the simulator creation and outcome sampling (no bootstrap, no fitting)

library(SensIAT)
library(dplyr)

# Must run log_link_simulation_and_bootstrap.R first to fit model_log
# This script assumes model_log is available in the environment

if (!exists("model_log")) {
    stop("Please run log_link_simulation_and_bootstrap.R first to create model_log")
}

cat("\n========== SINGLE-INDEX SIMULATOR BENCHMARK ==========\n\n")

outcome_model <- model_log$models$outcome
sampled_coef <- SensIAT:::get_bootstrap_coeffs(
    outcome_model,
...
# ============================================================================
# VERSION 3: OPTIMIZED (Option 2: Vectorized simulator)
# ============================================================================
make_parametric_single_index_simulator_v3_vectorized <- function(outcome_model, sampled_coef, covariate_mapping = NULL) {
    if (!inherits(outcome_model, "SensIAT::Single-index-outcome-model")) 
        stop("outcome_model must be a fitted single-index outcome model")
    ...
    # VECTORIZED: accepts vectors of inputs, returns vector of samples
    function(prev_outcomes, times, delta_times, newdata = NULL) {
        # Handle scalar or vector inputs
        if (!is.vector(prev_outcomes)) prev_outcomes <- c(prev_outcomes)
        if (!is.vector(times)) times <- c(times)
        if (!is.vector(delta_times)) delta_times <- c(delta_times)
        
        n_obs <- length(prev_outcomes)
        results <- numeric(n_obs)
        
        # Build full newdata frame at once instead of row-by-row
        if (is.null(newdata)) {
            nd <- data.frame(
        ...
        # Compute X_new once for all rows
        X_new <- tryCatch(
            model.matrix(model_terms, data = nd),
            error = function(e) stop("Failed to build model matrix for outcome newdata: ", conditionMessage(e))
        )
        
        # Compute all xb values at once (vectorized)
        xb_all <- as.vector(X_new %*% as.numeric(sampled_coef[colnames(X_new)]))
        
        # For each observation, sample from conditional distribution
        for (i in seq_len(n_obs)) {
            Fhat <- pcoriaccel_NW(Xb = lp0, Y = y_resp, xb = xb_all[i], y_seq = y_seq, h = bandwidth, kernel = kernel_type)
            Fhat_vec <- as.vector(Fhat)
            pmf <- pmax(0, diff(c(0, Fhat_vec)))
            if (sum(pmf) <= 0) {
                results[i] <- mean(y_resp, na.rm = TRUE)
            } else {
                pmf <- pmf / sum(pmf)
                results[i] <- sample(y_seq, size = 1, prob = pmf)
            }
        }
        
        if (n_obs == 1) return(results[1]) else return(results)
    }
}
...
benchmark_simulator <- function(simulator, n_obs = 200, version_name = "Version") {
    ...
    # Warm up
    try({
        if (version_name == "v3_vectorized") {
            simulator(prev_outcomes[1:5], times[1:5], delta_times[1:5])
        } else {
    ...
    if (version_name == "v3_vectorized") {
        # Vectorized version: call once with all observations
        results <- simulator(prev_outcomes, times, delta_times)
    } else {
...
t_build <- proc.time()
sim_v3 <- make_parametric_single_index_simulator_v3_vectorized(outcome_model, sampled_coef)
elapsed_v3_build <- (proc.time() - t_build)[[3]]
cat(sprintf("v3_vectorized built in %.4f s\n\n", elapsed_v3_build))
...
for (n_obs in c(50, 200, 500)) {
    ...
    result_v3 <- benchmark_simulator(sim_v3, n_obs, "v3_vectorized")
    
    speedup_v2 <- result_v1$elapsed / result_v2$elapsed
    speedup_v3 <- result_v1$elapsed / result_v3$elapsed
    
    cat(sprintf("\nSpeedup (v2 vs v1): %.2f x\n", speedup_v2))
    cat(sprintf("Speedup (v3 vs v1): %.2f x\n", speedup_v3))
}
