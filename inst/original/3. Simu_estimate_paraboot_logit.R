...
inner_ARC_SIR_sim_data_single_paraboot <- function(seed = 123,
                                                   data = data_temp,
                                                   N = 200,
                                                   End = 830,
    ...
    #############################################################
    {
      ...
      ############## The negative binomial model ##############
      {
        ...
        for(n_index in 1:N){
          ...
          if(is.na(newdat_2[n_index, ]$Prev_outcome) == T){
            Y2_sim[n_index] <- NA
          }else{
            temp <- NW(Xb = Xb, 
                       Y = Yi,
      ...
      ############## The negative binomial model ##############
      {
        ...
        for(n_index in 1:N){
          ...
          if(is.na(newdat_3[n_index, ]$Prev_outcome) == T){
            Y3_sim[n_index] <- NA
          }else{
          temp <- NW(Xb = Xb, Y = Yi,
      ...
      ############## The negative binomial model ##############
      {
        ...
        Y4_sim <- rep(NA, N)
        for(n_index in 1:N){
          if(is.na(newdat_4[n_index, ]$Prev_outcome) == T){
            Y4_sim[n_index] <- NA
          }else{
...
### Functions with Gauss - Legendre quadrature (binary outcome)
{
...
trt_group <- "control" # "treat" or "control"
range <- c(60 : 75)
weight_type <- "weight2"
...
# dataall <- readRDS(paste0('SIM_sim_data_', trt_group, '_binary_200_norm1'))

setwd("/uufs/chpc.utah.edu/common/home/u6049227/General_sensitivity/ARC_simu_result/parametric_sim_data")
dataall <- readRDS(paste0('SIM_sim_data_', trt_group, '_binary_200_logistic'))

# setwd("/uufs/chpc.utah.edu/common/home/u6049227/General_sensitivity/ARC_simu_result/single_index_sim_data")
# dataall <- readRDS(paste0('SIM_sim_data_', trt_group, '_binary_200_fixcoef'))
...
for(i in range){
  ...
  # generate parametric bootstrap data set
  if(trt_group == "treat"){
    set.seed(i*10) # for treat group
  }else{
    set.seed(i*30) # for control group
  }
  ...
  result_simu <- parallel::mclapply(Index_sim,
                                    inner_ARC_SIR_sim_data_single_paraboot,
                                    data = data,
                                    N = 200,
                                    End = 830,
  ...
  # temp <- inner_ARC_SIR_analyze_data_GL(data = result_simu[[200]][[1]],
  #                                       Alpha_seq = c(-0.8, -0.4, 0, 0.4, 0.8, 1.2), # "logit"
  #                                       link_type = "logit", # "log", "logit", "identity"
  #                                       SIM_model = "fixed_coef", # "norm1coef", "fixed_coef", "fixed_band"
  #                                       IF_type = weight_type)
  ...
  # Time difference of 2.029866 mins
  
  # setwd(paste0('/uufs/chpc.utah.edu/common/home/u6049227/General_sensitivity/ARC_simu_result/single_simu_logit_norm1/simu_',
  #              trt_group,
  #              '/para_boot_',
  #              weight_type))
  
  # setwd(paste0('/uufs/chpc.utah.edu/common/home/u6049227/General_sensitivity/ARC_simu_result/single_simu_logit_fixcoef/simu_',
  #              trt_group,
  #              '/para_boot_',
  #              weight_type))
  
  
  setwd(paste0('/uufs/chpc.utah.edu/common/home/u6049227/General_sensitivity/ARC_simu_result/logistic_simu_logit_norm1/simu_',
               trt_group,
               '/para_boot_',
               weight_type))

  saveRDS(result_trt, file = paste0('sim_paraboot_', save_group, '_logit_200_', i))
...
miss_index <- c()
for(i in 1:500){
 temp <- try({readRDS(file = paste0('sim_paraboot_', save_group, '_logit_200_', i))}, silent = TRUE)
 if(length(temp) == 1){
  miss_index <- c(miss_index, i) 
 }
...
# The final method:
# Logistic regression model + norm1: 
#. This can work, however, the coverage result is not well.

# Single index model + fix coef: single index model not converge 
# This cannot work because the single index model cannot be fitted

# Single index model + norm1: 
# Bad coverage

# Logistic regression model + fix coef: single index model not converge 
# This cannot work because the single index model cannot be fitted
