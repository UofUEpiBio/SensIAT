...
cat("\nTerm2 for first patient:\n")
print(result$influence[[1]]$term2[[1]])

cat("\nAll term2 values zero?:", all(sapply(result$influence[[1]]$term2, function(x) all(x == 0))), "\n")

cat("\nNumber of patients:", length(result$influence[[1]]$term2), "\n")

# Check term1 to see if at least that works
cat("\nTerm1 for first patient (first few observations):\n")
print(head(result$influence[[1]]$term1, 3))
